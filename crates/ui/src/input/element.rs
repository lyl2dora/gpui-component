...
impl Element for TextElement {
    ...
    fn paint(
        ...
        self.state.update(cx, |state, cx| {
            ...
            state.update_scroll_offset(prepaint.cursor_scroll_offset, cx);
            state.deferred_scroll_offset = None;
            // No unconditional cx.notify() here — paint is a read-only observation
            // endpoint. update_scroll_offset notifies only when values truly change.
        });
